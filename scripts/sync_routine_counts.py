...
def update_context(totals):
    ...
    if new == text:
        print('WARNING: snapshot block not found / unchanged in .routine-context.md')
...
PROMPT_TEMPLATE = """You are the scheduled remote agent for the A to E MCQ Bank (mord58562, Y4 Australian medical exam prep).
...
if [ ${{PIPESTATUS[0]}} -ne 0 ]; then
  ...
  echo 'the maintainer: investigate push path before re-enabling routine.'
  exit 1
fi
