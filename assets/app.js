...
(function () {
  ...
  // Combined admin check: legacy the maintainer profile OR signed-in cloud admin.
  ...
  function startQuiz() {
    ...
    // study mode (the maintainer's "either fix or hide" call - per-question timer
  ...
  function tick() {
    ...
    if (state.quiz.deadline) {
      ...
      // for the current question. Per the maintainer's call we do NOT show a Q
  ...
  async function submitReport() {
    ...
    if (res && res.ok) {
      ...
      status.textContent = "Couldn't reach the backend. Try again, or screenshot + email the maintainer.";
      status.classList.add("bad");
    }
  }

  // ── Audit dashboard (the maintainer only) ─────────────────────────────────────────
...

{
  ...
  function maybeShowHouseQuote() {
    // the maintainer's spec: fire reliably on every 50th UNIQUE answered question
